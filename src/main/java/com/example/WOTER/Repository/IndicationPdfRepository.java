...
public interface IndicationPdfRepository extends JpaRepository<SaldoEntity, Long> {
    @Query(value = """          
                           ...
                           and wt.tarif_type_id = 3)as tarif,
                                     (
                                         select to_char(max(case when rn = 2 then wi.date_calc end), 'DD.MM.YYYY')
                                         from (
                                             select indication, m3, summa, month_id, row_number() over (order by month_id desc) rn
                                       ...
                                       (
                                         select to_char(max(case when rn = 1 then wi.date_calc end), 'DD.MM.YYYY')
                                         from (
                                             select indication, m3, summa, month_id, row_number() over (order by month_id desc) rn
                                       ...
                                       (
                                         ...
                                         from (
                                             select indication, row_number() over (order by month_id desc) rn
                                       ...
                                       (
                                         ...
                                         from (
                                             select m3, row_number() over (order by month_id desc) rn
                                       ...
                                       (
                                         ...
                                         from (
                                             select summa, row_number() over (order by month_id desc) rn
